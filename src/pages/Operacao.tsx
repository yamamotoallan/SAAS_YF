...
const Operacao = ({ isWrapper = false }: { isWrapper?: boolean }) => {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const response = await api.operations.metrics() as OperationsMetrics;
            setData(response);
    ...
    return (
        <div className={`container animate-fade ${isWrapper ? 'is-wrapper pt-0' : ''}`}>
            ...
            <div className="ops-intelligence-row">
                {/* Productivity Ranking */}
                <div className="card productivity-card">
                    <div className="card-header">
                        <h3 className="text-h3"><Activity size={18} /> Ranking de Produtividade</h3>
                        <span className="text-caption">Itens concluídos por responsável</span>
                    </div>
                    <div className="productivity-list">
                        {data.productivityRanking?.map((user: ProductivityStat, index: number) => (
                            <div key={user.name} className="productivity-item">
                                <span className="rank-number">{index + 1}</span>
                                <div className="user-info">
                                    <span className="user-name">{user.name}</span>
                                    <span className="user-active-load">{user.active} ativos</span>
                                </div>
                                <div className="user-stats">
                                    <span className="count-value">{user.completed}</span>
                                    <span className="count-label">entregues</span>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="card risk-card">
                    <div className="card-header">
                        <h3 className="text-h3"><AlertTriangle size={18} /> Itens em Risco (SLA)</h3>
                        <span className="text-caption">Ações prioritárias preventivas</span>
                    </div>
                    <div className="risk-list">
                        {activeFlow.itemRisks && activeFlow.itemRisks.length > 0 ? (
                            activeFlow.itemRisks.map((item: ItemRisk) => (
                                <div key={item.id} className={`risk-item ${item.riskLevel}`}>
                                    <div className="risk-info">
                                        <span className="risk-title">{item.title}</span>
                                        <span className="risk-meta">{item.responsible} • {item.timeInStage}d no estágio</span>
                                    </div>
                                    <span className="risk-badge">{item.riskLevel === 'critical' ? 'Crítico' : 'Médio'}</span>
                                </div>
                            ))
                        ) : (
                            <div className="p-4 text-center text-muted text-small">
                                Nenhum item em risco imediato neste fluxo.
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flow-column">
                    <div className="card">
                        ...
                        <div className="flow-visual">
                            {activeFlow.stages.map((stage: StageMetric, index: number) => {
                        ...
                        <div className="diag-block">
                            ...
                            {activeFlow.bottlenecks && activeFlow.bottlenecks.length > 0 ? (
                                <ul className="diag-list">
                                    {activeFlow.stages.filter((s: StageMetric) => activeFlow.bottlenecks.includes(s.id)).map((s: StageMetric) => (
                                        <li key={s.id}>
