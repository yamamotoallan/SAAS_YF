...
router.put('/items/:id', checkRole(['admin', 'manager']), async (req: AuthRequest, res) => {
    try {
        ...
        const processItem = await prisma.processItem.update({
            ...
            data: {
                ...
                responsible: responsible as boolean,
...
router.get('/diagnosis', async (req: AuthRequest, res) => {
    try {
        ...
        // Fetch late operating items for friction analysis
        const lateItems = await prisma.operatingItem.findMany({
            where: {
                flow: { companyId: req.companyId as string },
                status: 'active',
        ...
        // Map Flow Types to Process Block Types
        ...
        const blockScores = blocks.map(block => {
            ...
            // Friction Analysis: Items of this category that are late
            const frictionItems = lateItems.filter(item => flowToProcessMap[item.flow.type] === block.type);
            const frictionScore = frictionItems.length;
