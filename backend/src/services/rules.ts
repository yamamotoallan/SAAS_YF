...
export const RulesService = {
    ...
    extractMetricValue: (metric: string, data: any): number | string | null => {
        ...
        if (metric === 'turnover') return Number(data.turnover || 0);
    ...
    triggerAction: async (rule: any, actualValue: any) => {
        ...
        const description = `A regra "${rule.name}" foi ativada. Valor atual: ${actualValue} (Critério: ${rule.operator} ${rule.value})`;

        // Race condition / Duplicate prevention: 
        // Check if an EXACT active alert already exists to avoid flooding
        ...
        if (existing) {
            console.log(`[RulesService] Alert already exists, skipping: ${title}`);
            return;
        }
