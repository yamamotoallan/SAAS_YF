...
router.get('/summary', async (req: AuthRequest, res) => {
    try {
        ...
        const prevEntries = await prisma.financialEntry.findMany({
            where: {
                companyId: req.companyId,
        ...
        // Cash available (all revenue minus all costs ever)
        ...
        res.json({
            ...
            revenueTrend,
            costTrend,
            operatingMonths: cashAvailable > 0 && costs > 0 ? Math.floor(cashAvailable / costs) : 0,
        });
    } catch (err) {
        console.error(err);
...
router.put('/:id', async (req: AuthRequest, res) => {
    try {
        const entry = await prisma.financialEntry.updateMany({
        ...
        if (entry.count === 0) { res.status(404).json({ error: 'Lançamento não encontrado' }); return; }
